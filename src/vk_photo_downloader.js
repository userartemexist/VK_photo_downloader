...
const VK_SELECTORS = {
    VIEWER_CONTENT: '.pv_content',
    VIEWER_LAYER: '.layer_photo',
...
const injectStyles = () => {
    ...
    const cursorSVG = `data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='32' height='32' viewBox='0 0 32 32'><line x1='16' y1='4' x2='16' y2='12' stroke='black' stroke-width='2'/><line x1='16' y1='20' x2='16' y2='28' stroke='black' stroke-width='2'/><line x1='4' y1='16' x2='12' y2='16' stroke='black' stroke-width='2'/><line x1='20' y1='16' x2='28' y2='16' stroke='black' stroke-width='2'/><circle cx='16' cy='16' r='2' fill='black'/></svg>`;
    ...
    style.textContent = `
        ...
        /* Custom Cursor */
        body.sf-selecting-mode { cursor: url('${cursorSVG}') 16 16, crosshair; }
        body.sf-selecting-mode a[href*="photo"] { cursor: url('${cursorSVG}') 16 16, crosshair; }
        ...
        .sf-toggle-label { flex: 1; cursor: pointer; text-align: center; margin: 0 10px; font-size: 14px; font-weight: normal; color: #666; transition: color 0.2s; }
        
        /* Active Label Colors */
        .sf-toggle-label.active-green { font-weight: bold; color: #4CAF50; }
        .sf-toggle-label.active-blue { font-weight: bold; color: #2a5885; }
        
        /* Track Logic */
        .sf-toggle-track { 
            width: 40px; height: 20px; background: #4CAF50; /* Green for Range */
            border-radius: 10px; position: relative; cursor: pointer; 
            border: none; transition: background 0.2s; 
        }
        .sf-toggle-track.active { 
            background: #2a5885; /* Blue for Pick */
        }
        
        .sf-toggle-thumb { 
            position: absolute; top: 2px; left: 2px; /* Default Left (Range) */
            width: 16px; height: 16px; background: #fff; 
            border-radius: 50%; transition: left 0.2s; 
        }
        .sf-toggle-track.active .sf-toggle-thumb { 
            left: 22px; /* Move Right (Pick) */
        }
...
const VKPhotoModule = {
    ...
    isProcessing: false, abortFlag: false, isFetchingMeta: false,
    ...
    isViewerOpen: function() {
        return !!document.querySelector(VK_SELECTORS.VIEWER_CONTENT) || !!document.querySelector(VK_SELECTORS.VIEWER_LAYER);
    },
    ...
    syncPanelVisibility: function() {
        ...
        // Если идет скачивание, панель должна быть видна
        if (this.isProcessing) return;
        
        const viewerOpen = this.isViewerOpen();
        
        // Soft Hide: Если просмотрщик открыт, скрываем панель
        if (viewerOpen && this.uiPanel.style.display !== 'none') {
            this.uiPanel.style.display = 'none';
        } 
        // Soft Show: Если просмотрщик закрыт, показываем панель обратно
        else if (!viewerOpen && this.uiPanel.style.display === 'none' && !this.abortFlag) {
    ...
    resetState: function(clearPrefix) {
        ...
        this.isPicking = false; this.isProcessing = false;
    ...
    initPanelEvents: function() {
         ...
         // FIX 3.2: Защита кнопок от нажатия при скачивании ИЛИ при открытом просмотрщике
         document.getElementById('sf-btn-a').onclick = () => { if(this.isProcessing || this.isViewerOpen()) return; if (this.mode === 'range') this.setMode('start'); else this.togglePicking(); };
         document.getElementById('sf-btn-b').onclick = () => { if(this.isProcessing || this.isViewerOpen()) return; if (this.mode === 'range') this.setMode('end'); else this.stopPicking(); };
         document.getElementById('sf-btn-run').onclick = () => { if(this.isProcessing || this.isViewerOpen()) return; this.runDownload(); };
         
         const track = document.getElementById('sf-toggle-track');
         const toggleHandler = () => { if(this.isProcessing || this.isViewerOpen()) return; this.switchMode(this.mode === 'range' ? 'pick' : 'range'); };
         track.onclick = toggleHandler;
         track.onkeydown = (e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); toggleHandler(); } };
         document.getElementById('lbl-range').onclick = () => { if (this.mode !== 'range') this.switchMode('range'); };
         document.getElementById('lbl-pick').onclick = () => { if (this.mode !== 'pick') this.switchMode('pick'); };
    },
    ...
    // Hard Hide: Manual close by user
    hidePanel: function() {
        this.clearStorage();
        this.resetState(false);
    ...
    initClickListener: function() {
        ...
        document.addEventListener('click', (e) => {
            // FIX 3.1: Убрана глобальная блокировка документа
            
            if (e.target.closest('#sf-dl-panel')) return;
            if (VKPhotoModule.isViewerOpen()) return; // Do not intercept if viewer is open
    ...
    setMode: function(type) {
        if (this.isProcessing) { this.resetState(false); }
    ...
    runDownload: function() {
        ...
        this.isProcessing = true; const btnA = document.getElementById('sf-btn-a'); const btnB = document.getElementById('sf-btn-b'); if (btnA) btnA.disabled = true; if (btnB) btnB.disabled = true; this.setRunButtonDisabled(true);
